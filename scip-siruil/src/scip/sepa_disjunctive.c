...
/* added by mbp28 */
SCIP_RETCODE SCIPoptsepaDisjunctive(
   SCIP* scip,
   SCIP_RESULT* result
)
{
   ...
   sepa = SCIPfindSepa(scip, "disjunctive");
   allowlocal = TRUE;
   /* end mbp28 */
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   if( !scip->set->optsepa_disjunctive )
      return SCIP_OKAY;
   ...
   for (i = 0; i < maxcuts; ++i)
   {
      ...
      /* add coefficients to cut */
      /* gizarp: note that here no "opt" is prepended to the cut name, as we do not have a proper opt method */
      SCIP_CALL( generateDisjCutSOS1(scip, sepa, rows, nrows, cols, ncols, ndisjcuts, MAKECONTINTEGRAL, sepadata->strengthen, cutlhs1, cutlhs2, bound1, bound2, simplexcoefs1, simplexcoefs2, cutcoefs, &row, &madeintegral) );
      if ( row == NULL )
         continue;
         ...
         || ( ! madeintegral && ( sepadata->maxrank == -1 || cutrank <= sepadata->maxrank ) ) )
      {
         ...
         if ( rownnonz > 0 && ! SCIPisInfinity(scip, -SCIProwGetLhs(row)) && ! SCIProwIsInLP(row) && SCIPisCutEfficacious(scip, NULL, row) )
         {
            ...
            /* start mbp28 */
            // SCIP_CALL( SCIPaddRow(scip, row, FALSE, &infeasible) );
            if( SCIPisOptCutNew(scip, row) || scip->set->optsepa_duplicating )
            {
               SCIP_CALL( SCIPaddOptPoolCut(scip, row) );
            }
            /* end mbp28 */
            SCIPdebug( SCIP_CALL( SCIPprintRow(scip, row, NULL) ) );
            if ( infeasible )

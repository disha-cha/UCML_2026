 ...
 */
static
SCIP_RETCODE generateOptOddCycleCut(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_SEPA*            sepa,               /**< separator */
   SCIP_SOL*             sol,                /**< given primal solution */
   SCIP_VAR**            vars,               /**< problem variables */
   ...
   )
{
   ...
   /* create cut */
   /* gizarp: added "opt" and SCIPgetNLPs(scip) to cutname*/
   (void) SCIPsnprintf(cutname, SCIP_MAXSTRLEN, "opt-oddcycle_%d_%d", sepadata->ncuts, SCIPgetNLPs(scip));
   SCIP_CALL( SCIPcreateEmptyRowSepa(scip, &cut, sepa, cutname, -SCIPinfinity(scip), (ncyclevars-1)/2.0, FALSE, FALSE, TRUE) );
   SCIP_CALL( SCIPcacheRowExtensions(scip, cut) );
   negatedcount = 0;
   ...
   if( SCIPisCutEfficacious(scip, sol, cut) )
   {
      ...
      // mbp28: SCIPaddRow -> SCIPaddOptPoolCut
      // SCIP_CALL( SCIPaddRow(scip, cut, FALSE, &infeasible) );
      ...
      {
         // mbp28: SCIPaddPoolCut -> SCIPaddOptPoolCut
         SCIP_CALL( SCIPaddOptPoolCut(scip, cut) );
         ...
         if( graph.nlevels > 0 && (sepadata->includetriangles || graph.nlevels > 1) )
         {
                    ...
                    && ncutslevel < maxcutslevel && !SCIPisStopped(scip); ++j )
            {
               ...
               if(success)
               {
                  ...
                  // mbp28: generateOddCycleCut -> generateOptOddCycleCut
                  SCIP_CALL( generateOptOddCycleCut(scip, sepa, sol, vars, nbinvars, graph.targetAdj[j], pred, ncyclevars,
                        incut, vals, sepadata, &graphdata, result) );
   ...
   )
{
           ...
           && !SCIPisStopped(scip); ++i )
   {
      ...
      if( success )
      {
         ...
         // mbp28: generateOddCycleCut -> generateOptOddCycleCut
         SCIP_CALL( generateOptOddCycleCut(scip, sepa, sol, vars, nbinvars, startnode, pred2, ncyclevars, incut, vals, sepadata, &graphdata, result) );
      }
   }
...
/** separation method */
static
SCIP_RETCODE separateoptOddCycles(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_SEPA*            sepa,               /**< separator */
   SCIP_SOL*             sol,                /**< given primal solution */
   SCIP_RESULT*          result              /**< pointer to store the result of the separation call */
   )
{
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   // mbp28: disable not running if small number of cuts.
   // /* only run if number of cuts already found is small enough */
   // if ( sepadata->cutthreshold >= 0 && SCIPgetNCutsFoundRound(scip) >= sepadata->cutthreshold )
   //    return SCIP_OKAY;
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   {
      SCIPdebugMsg(scip, "using GLS method for finding odd cycles\n");
      // mbp28: separateGLS -> separateOptGLS
      SCIP_CALL( separateOptGLS(scip, sepa, sepadata, sol, result) );
   }
   else
   {
      SCIPdebugMsg(scip, "using level graph heuristic for finding odd cycles\n");
      // mbp28: separateHeur -> separateOptHeur
      SCIP_CALL( separateOptHeur(scip, sepa, sepadata, sol, result) );
   }
...
SCIP_RETCODE SCIPoptsepaOddcycle(
   SCIP* scip,
   SCIP_RESULT* result
)
{  /*lint --e{715}*/
   ...
   /* start mbp28 */
   SCIP_SEPA* sepa;
   sepa = SCIPfindSepa(scip, "oddcycle");
   /* end mbp28 */
   ...
   // mbp28: separateOddCycles -> separateOptOddCycles
   SCIP_CALL( separateoptOddCycles(scip, sepa, NULL, result) );

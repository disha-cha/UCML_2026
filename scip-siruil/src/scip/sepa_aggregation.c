...
tryagain:
      ...
      if( SCIPisCutNew(scip, cut) || scip->set->optsepa_duplicating )
      {
         (*ncuts)++;
   ...
   )
{
   ...
   *cutoff = FALSE;
   ...
   if( cutnnz > 0 && SCIPisEfficacious(scip, cutefficacy) )
   {
      ...
      (void) SCIPsnprintf(cutname, SCIP_MAXSTRLEN, "opt-%s%" SCIP_LONGINT_FORMAT "_%d", cutclassname, SCIPgetNLPs(scip), *ncuts);
...
tryagain:
      ...
      /* end mbp28 */
      {
         ...
         //    SCIP_CALL( SCIPaddRow(scip, cut, FALSE, cutoff) );
         // }
         SCIP_CALL( SCIPaddOptPoolCut(scip, cut) );
         // end mbp28
      ...
      {
         /* release the row */
         // mbp28: it seems weird that this is in else
         SCIP_CALL( SCIPreleaseRow(scip, &cut) );
      }
   }
   ...
   *wastried = FALSE;
   ...
   while( naggrs <= maxaggrs )
   {
      ...
      *wastried = TRUE;
      ...
      /* start mbp28 */
      // add flowcover cut if successful.
      if ( flowcoversuccess )
      {
          // mbp28: addCut -> addOptCut
          SCIP_CALL( addOptCut(scip, sol, sepadata->flowcover, FALSE, cutcoefs, cutinds, cutnnz, cutrhs, cutefficacy, flowcovercutislocal, sepadata->dynamiccuts, cutrank, "flowcover", cutoff, ncuts, &cut) ); /*lint !e644*/
      }
      /* end mbp28 */
       ...
       */
      /* start mbp28 */
      // to bypass the overwrite, we have add
      // cutefficacy = flowcoverefficacy;
      cutefficacy = 0.0;
      /* end mbp28 */
      if( sepadata->sepcmir )
      ...
      {
         // mbp28: addCut -> addOptCut
         SCIP_CALL( addOptCut(scip, sol, sepadata->cmir, FALSE, cutcoefs, cutinds, cutnnz, cutrhs, cutefficacy, cmircutislocal, sepadata->dynamiccuts, cutrank, "cmir", cutoff, ncuts, &cut) ); /*lint !e644*/
      }
      /* end mbp28 */
...
/** added by mbp28: separateOptCuts */
/** searches for and adds c-MIR cuts that separate the given primal solution */
static
SCIP_RETCODE separateOptCuts(
   SCIP*                 scip,               /**< SCIP data structure */
   SCIP_SEPA*            sepa,               /**< the c-MIR separator */
   SCIP_SOL*             sol,                /**< the solution that should be separated, or NULL for LP solution */
   SCIP_Bool             allowlocal,         /**< should local cuts be allowed */
   ...
   )
{
   ...
   /* check which cuts should be separated */
   {
      ...
      // sepadata->sepcmir = cmirfreq > 0 ? (depth % cmirfreq) == 0 : cmirfreq == depth;
      // sepadata->sepflowcover = flowcoverfreq > 0 ? (depth % flowcoverfreq) == 0 : flowcoverfreq == depth;
      sepadata->sepcmir = scip->set->optsepa_cmir;
      sepadata->sepflowcover = scip->set->optsepa_flowcover;
      /* end mbp28*/
   }
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   for( r = 0; r < nnonzrows && ntries < maxtries && ncuts < maxsepacuts && !SCIPisStopped(scip); r++ )
   {
      ...
      oldncuts = ncuts;
      // mbp28: aggregation -> optaggregation
      SCIP_CALL( optaggregation(scip, &aggrdata, sepa, sol, allowlocal, rowlhsscores, rowrhsscores,
            roworder[r], maxaggrs, &wastried, &cutoff, cutinds, cutcoefs, FALSE, &ncuts) );
...
/* added by mbp28 */
SCIP_RETCODE SCIPoptsepaAggregation(
   SCIP* scip,
   SCIP_RESULT* result
)
{
   ...
   sepa = SCIPfindSepa(scip, "aggregation");
   allowlocal = TRUE;
   /* end mbp28 */
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   if( !scip->set->optsepa_cmir && !scip->set->optsepa_flowcover )
      return SCIP_OKAY;
   ...
   // mbp28: separateCuts -> separateOptCuts
   SCIP_CALL( separateOptCuts(scip, sepa, NULL, allowlocal, result) );

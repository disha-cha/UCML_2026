   ...
   )
{
   ...
   *lperror = FALSE;
        ...
        ++i )
   {
      ...
      /* skip special separators whose name begins with "#CS" */
   ...
   )
{
   ...
   int actdepth;
   int npricedcolvars;
   int i;  /* loop over separators to find #CS separators */
   SCIP_RESULT result;  /* avrech - for calling ml separator sepaexeclp - done */
   int store_maxcutsroot; /* mbp28, can temporarily store separating/maxcutsroot param */
   int store_maxcuts;  /* mbp28, can temporarily store separating/maxcuts param */
   ...
   *unbounded = (SCIPlpGetSolstat(lp) == SCIP_LPSOLSTAT_UNBOUNDEDRAY);
   ...
   while( !(*cutoff) && !(*unbounded) && !(*lperror) && (mustprice || mustsepa || delayedsepa) )
   {
      ...
      if( mustsepa )
      {
         ...
         {
            assert(set != NULL);
            /* mbp28, save temporarily, because overrideCutSelection might overwrite those values.. */
            SCIP_CALL( SCIPsetGetIntParam(set, "separating/maxcutsroot", &store_maxcutsroot) );
            SCIP_CALL( SCIPsetGetIntParam(set, "separating/maxcuts", &store_maxcuts) );
                 ...
                 ++i )
            {
               ...
               if( strncmp(SCIPsepaGetName(set->sepas[i]), "#CS", 3) ) /* strncp returns 0 for #CS (equality), cast as False */
                  continue;
//               printf("calling %s for cut selection\n", SCIPsepaGetName(set->sepas[i]));
            ...
            /* mbp28: reset params from temporary storage.. */
            assert(set != NULL);
            assert(messagehdlr != NULL);
            SCIP_CALL( SCIPsetSetIntParam(set, messagehdlr, "separating/maxcutsroot", store_maxcutsroot) );
            SCIP_CALL( SCIPsetSetIntParam(set, messagehdlr, "separating/maxcuts", store_maxcuts) );
...
//
// /** applies cuts in opt separation storage to LP and resolves LP, etc., added by mp28
//  *
...
//    SCIP_Bool root;
//    SCIP_Longint olddomchgcount;
//    SCIP_Bool anybndchgs;
//
//    assert(scip != NULL);
//    optsepastore = scip->optsepastore;
//    blkmem = scip->mem->probmem;
//    set = scip->set;
//    messagehdlr = scip->messagehdlr;
//    stat = scip->stat;
//    transprob = transprob;
//    origprob = scip->origprob;
//    primal = scip->primal;
//    tree = scip->tree;
//    reopt = scip->reopt;
//    lp = scip->lp;
//    branchcand = scip->branchcand;
//    eventqueue = scip->eventqueue;
//    eventfilter = scip->eventfilter;
//    cliquetable = scip->cliquetable;
...
//    // propagateagain* = FALSE;
//
//    /* check if there are any bound changes in the opt sepastore */
//    // todo: implement loop
//    anybndchgs = FALSE;
//
//    /* apply all cuts in opt sepastore to LP and clear storage */
//    assert(root); // we only considered root level for custom methods
//    SCIP_CALL( SCIPsepastoreApplyOptCuts(optsepastore, blkmem, set, stat,
//               transprob, origprob, tree, reopt, lp, branchcand, eventqueue,
//               eventfilter, cliquetable, cutoff) );
//
//    /* LP should have been solved before calling this.. */
//    assert(lp->flushed);
//    assert(lp->solved);
//    assert(SCIPlpGetSolstat(lp) == SCIP_LPSOLSTAT_OPTIMAL || SCIPlpGetSolstat(lp) == SCIP_LPSOLSTAT_UNBOUNDEDRAY);
//
...
//    if( !(*cutoff) )
//        {
//          // if there were any bound changes, set propagateagain to false.
//          if( anybndchgs )
//          {
//            *propagateagain = TRUE;
//          }
...
//          if( !(*lperror) )
//          {
//            if( SCIPprobAllColsInLP(transprob, set, lp) && SCIPlpIsRelax(lp) )
//            {
//              SCIP_CALL( SCIPnodeUpdateLowerboundLP(focusnode, set, stat, tree, transprob, origprob, lp) );
//              SCIP_CALL( updateEstimate(set, stat, tree, lp, branchcand) );
//
...
//
//
SCIP_RETCODE resolveForcedCuts(
   BMS_BLKMEM*           blkmem,             /**< block memory buffers */
   SCIP_SET*             set,                /**< global SCIP settings */
   SCIP_MESSAGEHDLR*     messagehdlr,        /**< message handler */
   SCIP_STAT*            stat,               /**< dynamic problem statistics */
...
SCIP_RETCODE SCIPapplyOptCutsToLP(
   BMS_BLKMEM*           blkmem,             /**< block memory buffers */
   SCIP_SET*             set,                /**< global SCIP settings */
   SCIP_MESSAGEHDLR*     messagehdlr,        /**< message handler */
   SCIP_STAT*            stat,               /**< dynamic problem statistics */
   ...
   SCIP_REOPT*           reopt,              /**< reoptimization data structure */
   SCIP_LP*              lp,                 /**< LP data */
   SCIP_SEPASTORE*       optsepastore,          /**< separation storage */
   SCIP_BRANCHCAND*      branchcand,         /**< branching candidate storage */
   SCIP_EVENTFILTER*     eventfilter,        /**< event filter for global (not variable dependent) events */
   SCIP_EVENTQUEUE*      eventqueue,         /**< event queue */
   SCIP_CLIQUETABLE*     cliquetable,        /**< clique table data structure */
...
*unbounded = (SCIPlpGetSolstat(lp) == SCIP_LPSOLSTAT_UNBOUNDEDRAY);
    ...
    || SCIPlpGetSolstat(lp) == SCIP_LPSOLSTAT_ITERLIMIT  || SCIPlpGetSolstat(lp) == SCIP_LPSOLSTAT_TIMELIMIT )
 {
    /* the found cuts are of no use, because the node is infeasible anyway (or we have an error in the LP) */
    SCIPmessagePrintVerbInfo(messagehdlr, set->disp_verblevel, SCIP_VERBLEVEL_HIGH,
               "found cuts are not getting applied  \n");

    SCIP_CALL( SCIPsepastoreClearCuts(optsepastore, blkmem, set, eventqueue, eventfilter, lp) );
 }
 else
 {
    /* apply found cuts */
    SCIPmessagePrintVerbInfo(messagehdlr, set->disp_verblevel, SCIP_VERBLEVEL_HIGH,
               "found cuts are getting applied  \n");

    SCIP_CALL( SCIPsepastoreApplyOptCuts(optsepastore, blkmem, set, stat, transprob, origprob, tree, reopt, lp,
          branchcand, eventqueue, eventfilter, cliquetable, cutoff) );

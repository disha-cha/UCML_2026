   ...
   )
{
   ...
   SCIP_CALL( mod2MatrixTransformContRows(scip, sol, sepadata, mod2matrix, allowlocal, maxslack) );
   ...
   )
{
   ...
   for( i = 0; i < cutnnz; )
   {
      ...
      if( REALABS(coef - floorcoef) > 0.1 )
      {
         ...
         primsol = SCIPgetSolVal(scip, sol, vars[k]);
...
static
SCIP_RETCODE generateOptZerohalfCut(
   SCIP*                 scip,               /**< scip data structure */
   SCIP_SOL*             sol,                /**< solution to separate, or NULL for LP solution */
   MOD2_MATRIX*          mod2matrix,         /**< mod 2 matrix */
   SCIP_SEPA*            sepa,               /**< zerohalf separator */
   ...
   )
{
   ...
   if( ! SCIPcutsTightenCoefficients(scip, cutislocal, cutcoefs, &cutrhs, cutinds, &cutnnz, &nchgcoefs) )
   {
      ...
      if( SCIPisEfficacious(scip, cutefficacy) )
      {
         ...
         /* create the cut */
         /* gizarp: added "opt" to cutname */
         (void) SCIPsnprintf(cutname, SCIP_MAXSTRLEN, "opt-zerohalf%" SCIP_LONGINT_FORMAT "_x%d", SCIPgetNLPs(scip), row->index);
         ...
         // start: mbp28
         // mbp28: SCIPisCutNew->SCIPisOptCutNew
         // if( SCIPisCutNew(scip, cut) )
         if( SCIPisOptCutNew(scip, cut) || scip->set->optsepa_duplicating )
         // end: mbp28
         {
            int pos = sepadata->ncuts++;
   ...
   )
{
   ...
   for( i = 0; i < mod2matrix->nrows; )
   {
      ...
      else
      {
         ...
         SCIP_CALL( generateZerohalfCut(scip, sol, mod2matrix, sepa, sepadata, allowlocal, row) );
...
// added by mbp28
static
SCIP_RETCODE mod2matrixPreprocessRowsOpt(
   SCIP*                 scip,               /**< scip data structure */
   SCIP_SOL*             sol,                /**< solution to separate, or NULL for LP solution */
   MOD2_MATRIX*          mod2matrix,         /**< the mod 2 matrix */
   SCIP_SEPA*            sepa,               /**< the zerohalf separator */
   ...
   )
{
   ...
   for( i = 0; i < mod2matrix->nrows; )
   {
      ...
      else
      {
         ...
         // mbp28: generateZerohalfCut -> generateOptZerohalfCut
         SCIP_CALL( generateOptZerohalfCut(scip, sol, mod2matrix, sepa, sepadata, allowlocal, row) );
   ...
   )
{
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   for( k = 0; k < MAXREDUCTIONROUNDS; ++k )
   {
      ...
      SCIP_CALL( mod2matrixPreprocessRows(scip, sol, &mod2matrix, sepa, sepadata, allowlocal) );
      assert(mod2matrix.nzeroslackrows <= mod2matrix.nrows);
   ...
   if( sepadata->ncuts > 0  )
   {
      ...
      SCIP_CALL( SCIPselectCuts(scip, sepadata->cuts, sepadata->randnumgen, sepadata->goodscore, sepadata->badscore,
            sepadata->goodmaxparall, sepadata->maxparall, sepadata->dircutoffdistweight, sepadata->efficacyweight, sepadata->objparalweight, 0.0, 0.0, // added another 0.0 for disregading lookahead score at this stage.
   ...
   )
{
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   for( k = 0; k < MAXREDUCTIONROUNDS; ++k )
   {
      ...
      assert(mod2matrix.nzeroslackrows <= mod2matrix.nrows);
      // mbp28: mod2matrixPreprocessRows -> mod2matrixPreprocessRowsOpt
      SCIP_CALL( mod2matrixPreprocessRowsOpt(scip, sol, &mod2matrix, sepa, sepadata, allowlocal) );
      assert(mod2matrix.nzeroslackrows <= mod2matrix.nrows);
   ...
   if( sepadata->ncuts > 0  )
   {
      ...
      for( i = 0; i < sepadata->ncuts; ++i )
      {
        ...
        if( SCIPisOptCutNew(scip, sepadata->cuts[i]) || scip->set->optsepa_duplicating )
          {
            SCIP_CALL( SCIPaddOptPoolCut(scip, sepadata->cuts[i]) );
          }
        // end mbp28
...
static
SCIP_DECL_SEPAEXECSOL(sepaExecsolZerohalf)
{
   assert(result != NULL);
   assert(sepa != NULL);
   assert(strcmp(SCIPsepaGetName(sepa), SEPA_NAME) == 0);
...
/* added by mbp28 */
SCIP_RETCODE SCIPoptsepaZerohalf(
   SCIP* scip,
   SCIP_RESULT* result
)
{
   ...
   sepa = SCIPfindSepa(scip, "zerohalf");
   allowlocal = TRUE;
   /* end mbp28 */
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   // mbp28: only run if switched on
   if( !scip->set->optsepa_zerohalf )
      return SCIP_OKAY;
   ...
   /* mbp28: doSeparation->doOptSeparation */
   SCIP_CALL( doOptSeparation(scip, sepa, NULL, result, allowlocal) );

   ...
   )
{
   ...
   *cutoff = FALSE;
   ...
   if( root )
   {
      // start mbp28: deprecate minorthoroot
      // maxparall = 1.0 - set->sepa_minorthoroot;
      // goodmaxparall = MAX(0.5, 1.0 - set->sepa_minorthoroot);
      maxparall = set->sepa_maxparallroot;
      goodmaxparall = set->sepa_goodmaxparallroot;
      // end mbp28
   }
   else
   {
      // start mbp28: deprecate minortho
      // maxparall = 1.0 - set->sepa_minortho;
      // goodmaxparall = MAX(0.5, 1.0 - set->sepa_minortho);
      maxparall = set->sepa_maxparall;
      goodmaxparall = set->sepa_goodmaxparall;
      // end mbp28;
   }

   /* call cut selection algorithm */
   /* start mbp28, add more control by parameterizing function with goodscorefac and badscorefac */
   // SCIP_CALL( SCIPselectCuts(set->scip, sepastore->cuts, sepastore->randnumgen, 0.9, 0.0, goodmaxparall, maxparall,
   //       set->sepa_dircutoffdistfac, set->sepa_efficacyfac, set->sepa_objparalfac, set->sepa_intsupportfac, set->sepa_lookaheadfac, // added by mbp28
   //       sepastore->ncuts, sepastore->nforcedcuts, maxsepacuts, &nselectedcuts) );
   SCIP_CALL( SCIPselectCuts(set->scip, sepastore->cuts, sepastore->randnumgen, set->sepa_goodscorefac, set->sepa_badscorefac, goodmaxparall, maxparall,
         set->sepa_dircutoffdistfac, set->sepa_efficacyfac, set->sepa_objparalfac, set->sepa_intsupportfac, set->sepa_lookaheadfac, // added by mbp28
         sepastore->ncuts, sepastore->nforcedcuts, maxsepacuts, &nselectedcuts) );
   /* end mbp28 */
...
/** adds opt cuts to the LP and clears the opt separation storages, added by mbp28 */
SCIP_RETCODE SCIPsepastoreApplyOptCuts(
   SCIP_SEPASTORE*       optsepastore,          /**< opt separation storage */
   BMS_BLKMEM*           blkmem,             /**< block memory */
   SCIP_SET*             set,                /**< global SCIP settings */
   SCIP_STAT*            stat,               /**< problem statistics */
   SCIP_PROB*            transprob,          /**< transformed problem */
   ...
   )
{
   ...
   *cutoff = FALSE;
   ...
   ncutsapplied = 0;
   noptcuts = SCIPsepastoreGetNCuts(optsepastore);

   /* apply all selected cuts */
   for( i = 0; i < noptcuts && !(*cutoff); i++ )
   {
       SCIP_ROW* cut;

       cut = optsepastore->cuts[i];
       SCIP_Bool applied = FALSE;
       if( !SCIProwIsModifiable(cut) && SCIProwGetNNonz(cut) == 1 )
       {
           SCIPsetDebugMsg(set, " -> applying opt cut <%s> as boundchange\n", SCIProwGetName(cut));
           SCIP_CALL( sepastoreApplyBdchg(optsepastore, blkmem, set, stat, transprob, origprob, tree, reopt, lp, branchcand,
               eventqueue, cliquetable, cut, &applied, cutoff) );
           assert(applied || !sepastoreIsBdchgApplicable(set, cut)); // ????
       }
       ...
       {
           /* add cut to the LP and update orthogonalities */
           SCIPsetDebugMsg(set, " -> applying opt cut <%s>\n", SCIProwGetName(cut));
           SCIP_CALL( sepastoreApplyCut(optsepastore, blkmem, set, eventqueue, eventfilter, lp, cut, depth, &ncutsapplied) );
       }
   }
   /* clear the separation storage and reset statistics for separation round */
...
/** resort all cuts in the separation storage, and set the proper nforcedcuts counter,
    ...
    */
SCIP_RETCODE SCIPoverrideCutSelection(
...
//      scip->sepastore->cuts[forcedcuts[i]] = cut2;
   ...
   /* set separating/maxcutsroot and separating/maxcuts params, watch out, they
   must be reset manually, unless SCIPoverrideCutSelection is called by a #CS
   separator, that separator is embedded between two function calls that make
   sure the parameters are reset correctly, and you don't need to worry about it.
    */
   assert(scip->set != NULL);
   assert(scip->messagehdlr != NULL);
   // could have a root distinction, but I am lazy..
   SCIP_CALL( SCIPsetSetIntParam(scip->set, scip->messagehdlr, "separating/maxcutsroot", nforcedcuts) );
   SCIP_CALL( SCIPsetSetIntParam(scip->set, scip->messagehdlr, "separating/maxcuts", nforcedcuts) );

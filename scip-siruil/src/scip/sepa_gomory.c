...
/* added by mbp28 */
SCIP_RETCODE SCIPoptsepaGomory(
   SCIP* scip,
   SCIP_RESULT* result
)
{  /*lint --e{715}*/
   ...
   for( i = 0; i < nrows && naddedcuts < maxsepacuts && !SCIPisStopped(scip) && !cutoff; ++i )
   {
      ...
      if( success )
      {
         ...
         else if( SCIPisEfficacious(scip, cutefficacy) )
         {
            ...
            if( c >= 0 )
               (void) SCIPsnprintf(cutname, SCIP_MAXSTRLEN, "opt-gom%" SCIP_LONGINT_FORMAT "_x%d", SCIPgetNLPs(scip), c);
            else
               (void) SCIPsnprintf(cutname, SCIP_MAXSTRLEN, "opt-gom%" SCIP_LONGINT_FORMAT "_s%d", SCIPgetNLPs(scip), -c-1);
            ...
            if( cutnnz == 1 )
            {
                ...
                */
                ...
                /* flush all changes before adding the cut */
               SCIP_CALL( SCIPflushRowExtensions(scip, cut) );
               // start mbp28: add also bound changes to cutpool not directly to sepastore
               // SCIP_CALL( SCIPaddRow(scip, cut, TRUE, &cutoff) );

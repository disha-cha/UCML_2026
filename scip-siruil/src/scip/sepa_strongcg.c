...
/* added by mbp28 */
SCIP_RETCODE SCIPoptsepaStrongcg(
   SCIP* scip,
   SCIP_RESULT* result
)
{  /*lint --e{715}*/
   ...
   sepa = SCIPfindSepa(scip, "strongcg");
   allowlocal = TRUE;
   /* end mbp28 */
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   if( !scip->set->optsepa_strongcg )
      return SCIP_OKAY;

   // commented out by mbp28 mbp28
   // /* only call the strong CG cut separator a given number of times at each node */
   // if( (depth == 0 && sepadata->maxroundsroot >= 0 && ncalls >= sepadata->maxroundsroot)
   //    || (depth > 0 && sepadata->maxrounds >= 0 && ncalls >= sepadata->maxrounds) )
   //    return SCIP_OKAY;
   ...
   *result = SCIP_DIDNOTFIND;
   ...
   for( i = 0; i < nrows && ncuts < maxsepacuts && !SCIPisStopped(scip) && *result != SCIP_CUTOFF; ++i )
   {
      ...
      if( SCIPisEfficacious(scip, cutefficacy) )
      {
         ...
         if( c >= 0 )
            (void) SCIPsnprintf(cutname, SCIP_MAXSTRLEN, "opt-scg%" SCIP_LONGINT_FORMAT "_x%d", SCIPgetNLPs(scip), c);
         else
            (void) SCIPsnprintf(cutname, SCIP_MAXSTRLEN, "opt-scg%" SCIP_LONGINT_FORMAT "_s%d", SCIPgetNLPs(scip), -c-1);

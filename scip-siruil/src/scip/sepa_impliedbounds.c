   ...
   )
{
   ...
   *cutoff = FALSE;
   ...
   if( SCIPisEfficacious(scip, activity - rhs) )
   {
      ...
      /* create cut */
      (void) SCIPsnprintf(cutname, SCIP_MAXSTRLEN, "implbd%" SCIP_LONGINT_FORMAT "_%d", SCIPgetNLPs(scip), *ncuts);
   ...
   )
{
   ...
   *cutoff = FALSE;
   ...
   if( SCIPisEfficacious(scip, activity - rhs) )
   {
      ...
      (void) SCIPsnprintf(cutname, SCIP_MAXSTRLEN, "opt-implbd%" SCIP_LONGINT_FORMAT "_%d", SCIPgetNLPs(scip), *ncuts);
      SCIP_CALL( SCIPcreateEmptyRowSepa(scip, &cut, sepa, cutname, -SCIPinfinity(scip), rhs, FALSE, FALSE, TRUE) );
      SCIP_CALL( SCIPcacheRowExtensions(scip, cut) );
      SCIP_CALL( SCIPaddVarToRow(scip, cut, var1, val1) );
      SCIP_CALL( SCIPaddVarToRow(scip, cut, var2, val2) );
   ...
   for( i = 0; i < nfracs; i++ )
   {
      ...
      for( j = 0; j < nimpl; j++ )
      {
         ...
         if( impltypes[j] == SCIP_BOUNDTYPE_UPPER )
         {
            ...
            {
               /* add cut if violated */
               SCIP_CALL( addOptCut(scip, sepa, 1.0, implvars[j], solval, (ub - implbounds[j]), fracvars[i], fracvals[i],
                     ub, cutoff, ncuts) );
               if ( *cutoff )
                  return SCIP_OKAY;
            }
         ...
         else
         {
            ...
            {
               /* add cut if violated */
               SCIP_CALL( addOptCut(scip, sepa, -1.0, implvars[j], solval, (implbounds[j] - lb), fracvars[i], fracvals[i],
                     -lb, cutoff, ncuts) );
               if ( *cutoff )
                  return SCIP_OKAY;
            }
      ...
      for( j = 0; j < nimpl; j++ )
      {
         ...
         if( impltypes[j] == SCIP_BOUNDTYPE_UPPER )
         {
            ...
            {
               /* add cut if violated */
               SCIP_CALL( addOptCut(scip, sepa, 1.0, implvars[j], solval, (implbounds[j] - ub), fracvars[i], fracvals[i],
                     implbounds[j], cutoff, ncuts) );
               if ( *cutoff )
                  return SCIP_OKAY;
            }
         ...
         else
         {
            ...
            {
               /* add cut if violated */
               SCIP_CALL( addOptCut(scip, sepa, -1.0, implvars[j], solval, (lb - implbounds[j]), fracvars[i], fracvals[i],
                     -implbounds[j], cutoff, ncuts) );
               if ( *cutoff )
                  return SCIP_OKAY;
            }
...
/* added by mbp28 */
SCIP_RETCODE SCIPoptsepaImpliedbounds(
   SCIP* scip,
   SCIP_RESULT* result
)
{  /*lint --e{715}*/
   ...
   sepa = SCIPfindSepa(scip, "impliedbounds");
   /* end mbp28 */
   ...
   *result = SCIP_DIDNOTRUN;
   ...
   if( !scip->set->optsepa_impliedbounds )
      return SCIP_OKAY;
   ...
   /* call the cut separation */
   SCIP_CALL( optseparateCuts(scip, sepa, NULL, solvals, fracvars, fracvals, nfracs, &cutoff, &ncuts) );

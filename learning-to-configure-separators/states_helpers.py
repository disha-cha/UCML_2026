...
def computeSepaFeatures1(model, round_num=0):
    """
    Separator cumulative statistics (per separator).
    In the authors' patched SCIP/PySCIPOpt this comes from:
        model.getSepaCumulatedStatics()

    If not available, return zeros so the pipeline still runs.
    """
    features = []

    # If method not available, return zero stats (still lets you build states)
    if not hasattr(model, "getSepaCumulatedStatics"):
        for sepa_name in SCIP_CUT_IDENTIFIERS_TO_NUMS.keys():
            features.append({
                "#calls": 0.0,
                "time": 0.0,
                "#cuts": 0,
                ...
                "round_num": round_num,
                "name": sepa_name,
                "note": "fallback_no_getSepaCumulatedStatics"
            })
        return features

    stats = model.getSepaCumulatedStatics()
    for sepa_name in SCIP_CUT_IDENTIFIERS_TO_NUMS.keys():
        st = stats.get(sepa_name, {})
        features.append({
            "#calls": st.get("#calls", 0.0),
            "time": st.get("time", 0.0),
            "#cuts": st.get("#cuts", 0),
            "#cutoffs": st.get("#cutoffs", 0),
